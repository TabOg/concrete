...
fn out_variance(
    ...
    match op {
        ...
        Operator::Dot {
            ...
            let variance_iterator: Box<dyn Iterator<Item = SymbolicVariance>> = if inputs.len() == 1
            {
                Box::new(std::iter::repeat(
                    out_variances[inputs.iter().next().unwrap().0],
                ))
            } else {
                Box::new(inputs.iter().map(|n| out_variances[n.0]))
            };
            weights
                .values
                .iter()
                .zip(variance_iterator)
