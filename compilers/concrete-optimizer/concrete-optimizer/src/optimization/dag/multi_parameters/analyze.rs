...
#[allow(unused)]
struct ComposedInput<'a> {
    dag: &'a VariancedDag,
    id: OperatorIndex,
    from: Vec<OperatorIndex>,
}

struct ComposedInputMut<'a> {
    dag: &'a mut VariancedDag,
    id: OperatorIndex,
    from: Vec<OperatorIndex>,
}

impl<'a> ComposedInputMut<'a> {
    fn apply(self) {
        let maxed_variance = self
            .from
            .into_iter()
            .map(|id| self.dag.get_operator(id).variance().clone())
            .reduce(|acc, var| acc.partition_wise_max(&var))
            .unwrap();
        let mut input = self.dag.get_operator_mut(self.id);
...
impl VariancedDag {
    fn try_from_partitioned(partitioned: PartitionedDag) -> Result<Self> {
        ...
        for forward_count in 0..MAX_FORWARDING {
            // The noise gets gets computed from inputs down to outputs.
            if varianced.forward_noise() {
                break;
            }
            // If the noise is forwarded for the first time, we check that the dag is composable.
            if forward_count == 0 {
            ...
            assert!(
                forward_count != MAX_FORWARDING,
                "Forwarding of noise did not reach a fixed point."
            );
        }

        // We return the varianced dag.
        Ok(varianced)
    }

    #[allow(unused)]
    fn get_operators_iter(&self) -> impl Iterator<Item = VariancedDagOperator<'_>> {
        self.dag.get_indices_iter().map(|i| self.get_operator(i))
    }

    fn get_operators_iter_mut(&mut self) -> impl Iterator<Item = VariancedDagOperatorMut<'_>> {
        struct Iter<'a> {
            dag: &'a mut VariancedDag,
            range: Range<usize>,
        }
        ...
        impl<'a> Iterator for Iter<'a> {
            type Item = VariancedDagOperatorMut<'a>;
            ...
            fn next(&mut self) -> Option<Self::Item> {
                ...
                self.range = (start + 1)..end;
                unsafe {
                    Some(std::mem::transmute::<
                        VariancedDagOperatorMut<'_>,
                        VariancedDagOperatorMut<'a>,
                    >(
                        self.dag.get_operator_mut(OperatorIndex(start))
                    ))
                }
            }
        }
        Iter {
            range: 0..self.dag.len(),
            dag: self,
        }
    }

    #[allow(unused)]
    fn get_composed_inputs_iter(&self) -> impl Iterator<Item = ComposedInput<'_>> {
        self.dag
            .composition
            .clone()
            .into_iter()
            .map(move |(id, from)| ComposedInput {
                dag: self,
                id,
                from,
            })
    ...
    fn get_composed_inputs_iter_mut(&mut self) -> impl Iterator<Item = ComposedInputMut<'_>> {
        struct Iter<'a> {
            dag: &'a mut VariancedDag,
            iter: std::collections::hash_map::IntoIter<OperatorIndex, Vec<OperatorIndex>>,
        }

        impl<'a> Iterator for Iter<'a> {
            type Item = ComposedInputMut<'a>;

            fn next(&mut self) -> Option<Self::Item> {
                self.iter.next().map(|(to, from)| unsafe {
                    std::mem::transmute::<ComposedInputMut<'_>, ComposedInputMut<'a>>(
                        ComposedInputMut {
                            dag: self.dag,
                            id: to,
                            from,
        ...
        Iter {
            iter: self.dag.composition.clone().into_iter(),
            dag: self,
        }
    }
    ...
    fn apply_composition_rules(&mut self) {
        for compo in self.get_composed_inputs_iter_mut() {
            compo.apply();
        }
    }
    ...
    fn forward_noise(&mut self) -> bool {
        ...
        for mut operator in self.get_operators_iter_mut() {
                ...
                .operator
            {
                ...
                Operator::Dot {
                    ...
                    let variance_iterator: Box<dyn Iterator<Item = OperatorVariance>> =
                        if inputs.len() == 1 {
                            ...
                            Box::new(operator.get_inputs_iter().map(|n| n.variance().clone()))
                        };

                    weights.values.iter().zip(variance_iterator).fold(
                        SymbolicVariance::ZERO,
                        |acc, (weight, var)| {
    ...
    fn check_composability(&self) -> Result<()> {
        self.get_composed_inputs_iter()
            .flat_map(|comp| comp.from.into_iter())
...
pub mod tests {
    ...
    #[test]
    fn test_composition_with_many_levelled_circuits() {
        // Forces many forwarding iterations
        let precisions = [2u8, 3, 4, 5, 6, 7, 8, 9, 10];
        let mut dag = unparametrized::Dag::new();
        let f_inp = dag.add_input(1, Shape::number());
        let f_lev = dag.add_levelled_op(
            [f_inp],
        ...
        let f_oup = dag.add_lut(f_lev, FunctionTable::UNKWOWN, 2);
        let mut r_oup = f_oup;
        for prec in precisions {
            let inp = dag.add_input(prec, Shape::number());
