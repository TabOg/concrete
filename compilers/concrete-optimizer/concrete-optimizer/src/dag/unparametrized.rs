use crate::dag::operator::{
    DotKind, FunctionTable, LevelledComplexity, Operator, OperatorIndex, Precision, Shape, Weights,
};
use std::{
...
impl<'dag> DagOperator<'dag> {
    ...
    pub fn get_inputs_iter(&self) -> impl Iterator<Item = DagOperator<'dag>> + '_ {
        self.operator
            .get_inputs_iter()
...
impl<'dag> DagBuilder<'dag> {
    ...
    pub fn add_dot(
        ...
        let kind = if input_shape.is_number() && inputs_shape == weights.shape {
            DotKind::Simple
...
impl CompositionRules {
    ...
    pub(crate) fn update_index(&mut self, old_to_new_map: &[usize]) {
        let old_keys = self.0.keys().map(|n| n.0).collect::<Vec<usize>>();
        for to in old_keys {
            let new_to = OperatorIndex(old_to_new_map[to]);
            let new_froms = self
                .0
                .remove(&OperatorIndex(to))
                .unwrap()
                .into_iter()
                .map(|from| old_to_new_map[from.0])
                .map(OperatorIndex)
                .collect();
            let _ = self.0.insert(new_to, new_froms);
        }
    }
}
...
mod tests {
    use super::*;
    use crate::dag::operator::{ClearTensor, Shape};

    #[test]
    fn test_dot_kind_simple() {
        let mut dag = Dag::new();
        let input1 = dag.add_input(1, Shape::number());
        let input2 = dag.add_input(1, Shape::number());
        let dot = dag.add_dot([input1, input2], Weights::vector([1, 2]));
        let node = dag.get_operator(dot);
        assert!(matches!(
            node.operator,
            Operator::Dot {
                kind: DotKind::Simple,
    ...
    #[test]
    fn test_dot_kind_tensor() {
        let mut dag = Dag::new();
        let input = dag.add_input(1, Shape::vector(2));
        let dot = dag.add_dot([input], Weights::vector([1, 2]));
        let node = dag.get_operator(dot);
        assert!(matches!(
            node.operator,
            Operator::Dot {
    ...
    fn test_dot_kind_broadcast() {
        ...
        let dot = dag.add_dot([input], Weights::vector([1, 2]));
        let node = dag.get_operator(dot);
        let exp_shape = Shape::vector(2);
        assert!(
            matches!(node.operator, Operator::Dot { kind: DotKind::Broadcast{shape}, .. } if *shape == exp_shape)
        );
    }

    #[test]
    fn test_dot_kind_broadcast_scalar_mul() {
        let mut dag = Dag::new();
        let input = dag.add_input(
            1,
            Shape {
        ...
        let dot = dag.add_dot([input], Weights::number(1));
        let node = dag.get_operator(dot);
        let exp_shape = Shape {
            dimensions_size: vec![2, 2],
        };
    ...
    fn test_dot_kind_compatible() {
        ...
        let input1 = dag.add_input(1, Shape::vector(1));
        let input2 = dag.add_input(1, Shape::vector(1));
        let dot = dag.add_dot([input1, input2], weights);
        let node = dag.get_operator(dot);
        assert!(matches!(
            node.operator,
            Operator::Dot {
    ...
    #[test]
    #[should_panic(expected = "Unsupported or invalid dot operation")]
    fn test_dot_kind_unsupported() {
        let mut dag = Dag::new();
        let input1 = dag.add_input(1, Shape::number());
        let input2 = dag.add_input(1, Shape::number());
        let input3 = dag.add_input(1, Shape::number());
        let dot = dag.add_dot([input1, input2, input3], Weights::vector([1, 2]));
        let node = dag.get_operator(dot);
        assert!(matches!(
            node.operator,
            Operator::Dot {
